...
global mainInst;global cmaesInst;global abcInst;global LshadeInst;
for func = 5:5
    filename=['var' num2str(func) '.mat'];
    genname=['generations' num2str(func) '.mat'];
    timename=['time' num2str(func) '.mat'];
%**************************************************************************
...
%**************************************************************************	
    ...
    for run=1:runs
        tic;
        load(filename);
        load("nfes_multiEA_var.mat");load(timename);
        load(genname);
        if gen{run}
            gen{run}=[];
        end
        ...
        while mainInst.budget > mainInst.nfes
            ...
            %% Get the online probability vector
			% Get 1000 samples from the bootstrap distribution
            samples1 = cdf_100(predict_a);
            samples2 = cdf_100(predict_c);
            samples3 = cdf_100(predict_L);
			% Stack them together to form the scenarios set
            scenarios = [samples1' samples2' samples3'];
            [bst_val_ks,bst_ind_ks] = min(scenarios,[],2);
            Pr_ks = [sum(bst_ind_ks==1),sum(bst_ind_ks==2),sum(bst_ind_ks==3)];
            Pr_ks= Pr_ks./size(bst_ind_ks,1);
            %% Get the final probability vector (offline prob vector is set to ones)
            Final = Pr_ks;
            Final=cumsum(Final);
            temp = rand(1);
            selected = find(Final >= temp, 1,"first");
            ...
            if selected==1
                ...
                if abs(abcInst.BestCost(end) - optimum) <= 1e-8&& nfes_multiEA_var{func}(run)==0
            ...
            if selected==2
                ...
                if abs(cmaesInst.fmin(end) - optimum) <= 1e-8&& nfes_multiEA_var{func}(run)==0
                    nfes_multiEA_var{func}(run) = mainInst.nfes;
                end
            end
            if selected==3
                lshade;
				% if bsf is smaller than optimum, stop
                if abs(LshadeInst.bsf_fit_var(end) - optimum) <= 1e-8 && nfes_multiEA_var{func}(run)==0
                    nfes_multiEA_var{func}(run) = mainInst.nfes;
                end
            end
