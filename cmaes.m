...
% 
% Note: The code is downloaded from http://cma.gforge.inria.fr/cmaes.m
% 		
%		The main computational step is preserved while a benchmark is added to it and 
% 		it should be called by a driver function(cmaes_cmaesOnce.m) 
% 		to achieve running generation by generation.
% *******************************************************
...
global bias;
fitnessfun = @(fitfun,input)cec13_func(input, str2num(fitfun))-bias(str2num(fitfun));
...
defopts.MaxFunEvals  = '10000*N  % maximal number of fevals';
...
defopts.LBounds = '-100 % lower bounds, scalar or Nx1-vector'; 
defopts.UBounds = '100  % upper bounds, scalar or Nx1-vector'; 
...
% 	  'before resume']);
  ...
  if myevalbool(opts.EvalInitialX)
    fitness.hist(1)=fitnessfun(fitfun, xmean); 
    fitness.histsel(1)=fitness.hist(1);
%     counteval = counteval + 1;   %Commented by Ryy
...
%   end
  ...
  if flgEvalParallel
      ...
      fitness.raw = fitnessfun(fitfun, arxvalid); 
  ...
  for k=find(isnan(fitness.raw)), 
    ...
    while isnan(fitness.raw(k))
      ...
      fitness.raw(k) = fitnessfun(fitfun, arxvalid(:,k)); 
      tries = tries + 1;
...
%   end
  ...
  if mu == 1
    ...
    % fmean = fitnessfun(fitfun, xintobounds(xmean, lbounds, ubounds), varargin{:});
...
if length(stopflag) > sum(strcmp(stopflag, 'stoptoresume')) % final stopping
  out.solutions.mean.f = ...
      fitnessfun(fitfun, xintobounds(xmean, lbounds, ubounds));
  counteval = counteval + 1;
  out.solutions.mean.evals = counteval;
...
%added by LWW-------------------------------
% fi_star = -1400:100:1400;
% fi_star(find(fi_star == 0)) = [];
% bias = fi_star(str2num(fitfun));
% if flgdisplay && mod(counteval-1,100) == 0
    %num2str((fitness.hist(1))-bias)
   fprintf('|CMAES fit_val|: %f |#Iteration|: %d |nfes| %d\n',fitness.hist(1), countiter,counteval);
% end
%--------------------------------------------------------------
